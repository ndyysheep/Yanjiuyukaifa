...
public class DeviceDao {
	...
	{
		//构造sql语句，根据传递过来的条件参数
		String car_code = data.getParam().has("car_code")
				? data.getParam().getString("car_code") : null;
		String vehicle_type = data.getParam().has("vehicle_type")
				? data.getParam().getString("vehicle_type") : null;
		String illegal_status= data.getParam().has("illegal_status")
				? data.getParam().getString("illegal_status") : null;
		String capture_time= data.getParam().has("capture_time")
				? data.getParam().getString("capture_time") : null;
		String speed= data.getParam().has("speed")
				? data.getParam().getString("speed") : null;
	...
	{
		//构造sql语句，根据传递过来的条件参数
		String id = data.getParam().has("id") ?
				data.getParam().getString("id") : null;
		String car_code = data.getParam().has("car_code")
				? data.getParam().getString("car_code") : null;
		String vehicle_type = data.getParam().has("vehicle_type")
				? data.getParam().getString("vehicle_type") : null;
		...
		String speed= data.getParam().has("speed")
				? data.getParam().getString("speed") : null;
		String lane_name= data.getParam().has("lane_name")
				? data.getParam().getString("lane_name") : null;
	...
	public void GPSRecord(Data data, JSONObject json) throws JSONException
	{
		...
		/*--------------------数据操作 开始--------------------*/
		Db queryDb = new Db(dbName);
		String sql = "select count(*) as total from "+relationName+" where gpsTime between '"
				+timeFrom+"' and '"+timeTo+"'";
		showDebug("[queryRecord]构造的SQL语句是：" + sql);
...
//		showDebug(midSql);
	...
	public String createGetQueryRecordSql(Data data) throws JSONException {
		...
		if (checkParamValid(param, "time_from")
				&& checkParamValid(param, "time_to"))
		{

			if (!where.isEmpty())
			{
				where += " and create_time between '" + param.getString("time_from")
						+ "' and '" + param.getString("time_to") + "'";
			}
			else
			{
				where = "create_time between '" + param.getString("time_from")
						+ "' and '" + param.getString("time_to") + "'";
			}
	...
	public void getExportDeviceRecordToExcel(JSONObject json, Data data)
	...
	public  void getExportDeviceRecordToFile(JSONObject json, Data data)
			throws JSONException
	{

	}

	public void saveUploadFileRecord(JSONObject json, Data data)
			throws JSONException, SQLException
	{
		...
		String file_path_name=jsonFileList.getJSONObject(0).has("file_path_name")
				?jsonFileList.getJSONObject(0).get("file_path_name").toString():null;

		String visual_path_name=jsonFileList.getJSONObject(0).has("file_url_name")
				?jsonFileList.getJSONObject(0).get("file_url_name").toString():null;
