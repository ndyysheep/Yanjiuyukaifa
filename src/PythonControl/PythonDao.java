...
//        updateDb.close();
    ...
    private void imageAttachment(Data data, JSONObject json) throws JSONException {
        /*--------------------获取变量 开始--------------------*/
        JSONObject param = data.getParam();
        int resultCode = 0;
        String resultMsg = "ok";
        String target = workingDirectory + "\\src\\PythonControl\\Data\\image_ID"+"\\test.png";
        /*--------------------获取变量 完毕--------------------*/
        /*--------------------数据操作 开始--------------------*/
        // 构造sql语句，根据传递过来的查询条件参数
        // 首先分析json里有多少文件，多个文件需要用循环构造多个sql语句
...
//        updateDb.close();
    ...
    public void getBack(Data data,JSONObject json) {
        ...
        try {
            ...
            String numberName = (new SimpleDateFormat("yyyyMMddHHmmssSSS")).format(new Date())+".mp4";
            String virtual_path = "\\upload\\Back\\"+numberName;

            myFile.copyFile(file_path,workingDirectory+virtual_path);
            myFile.getBackResult(work_path+"\\results.txt",json);
    ...
    public void getImageCarId(Data data,JSONObject json) {


        try {
            FileManager myFile = new FileManager();
            imageAttachment(data,json);
            String pythonScriptPath = "Image_ID.py";
            String directUrl = workingDirectory+"\\src\\PythonControl";
            // 构建命令
            String[] command = {selectionPath, pythonScriptPath};
            ProcessBuilder pb = new ProcessBuilder(command);
            ...
            String work_path = workingDirectory+"\\src\\PythonControl\\Data\\Image_ID";

            String file_path =  work_path+"\\output.png";
            String numberName = (new SimpleDateFormat("yyyyMMddHHmmssSSS")).format(new Date())+".png";
            String virtual_path = "\\upload\\Image_ID\\"+numberName;

            myFile.copyFile(file_path,workingDirectory+virtual_path);
            myFile.getImgCarIdResult(work_path+"\\results.txt",json);
            json.put("file_path", virtual_path);
            json.put("result_code", exitCode); // 返回0表示正常，不等于0就表示有错误产生，错误代码
            System.out.println("进程执行完成，退出码: " + exitCode);
        } catch (IOException e) {
    ...
    public static void main(String[] arg){
            ...
            FileManager myFile = new FileManager();
            myFile.getImgCarIdResult("D:\\.Probe0311\\研究与开发实践\\Opencv\\src\\PythonControl\\Data\\Image_ID\\results.txt",new JSONObject());
    }
}
