...
public class ServletActionRecordInfo extends HttpServlet {

	String module= "illegal";
	String sub="data";
	Data data=null;
	...
	public void showDebug(String msg)
	{
		System.out.println("["+(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"))
				.format(new Date())+"]["+module+"/"+sub+"/ServletAction]"+msg);
	}
		...
		else
		{
			...
			if (action.equals("get_illegal_record"))
			{
				...
				try {
					...
					getIllegalRecord(request, response, json);
			...
			if (action.equals("delete_illegal_record"))
			{
				...
				try {
					...
					deleteIllegalRecord(request, response, json);
	...
	/*========================================函数分流 结束========================================*/
	/*========================================公共函数 开始========================================*/
	private Data getPageParameters(HttpServletRequest request, HttpServletResponse response)
			...
			throws JSONException
	{
		...
		//如果有参数就另外处理
		if(data.getParam().has("ajax")
				&& data.getParam().getString("ajax").equals("true"))
		{
			isAjax=true;
		};
		...
		else
		{
			...
			String errorNo="0";
			String errorMsg="ok";
			String url = module+"/"+sub+"/result.jsp?action="+action
					+"&result_code="+errorNo+ "&result_msg=" + errorMsg;
			...
			throws JSONException, SQLException
	{
		...
		String filePath=rootPath+"\\\\teach\\\\"+(new SimpleDateFormat("yyyyMMddHH"))
				.format(new Date());
		String rootUrl="/upload";
		String filePathUrl=rootUrl+"/teach/"+(new SimpleDateFormat("yyyyMMddHH"))
				.format(new Date());
				...
				for (Iterator<FileItem> iter = list.iterator(); iter.hasNext();)
				{
					...
					else
					{
						...
						if(!fileName.isEmpty())
						{
							...
							objectId="UPLOAD_"+(new SimpleDateFormat("yyyyMMddHHmmssSSS"))
									.format(new Date());	//生成一个流水号附件id给前端
						...
						showDebug("[uploadFileAttachment]存到：fileName="+fileName+",filePath="
								+filePath+",fileSize="+fileSize+",fileUrl="+fileUrl);
					}
